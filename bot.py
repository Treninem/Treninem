...
def resolve_pack_query(query: str) -> tuple[dict[str, Any] | None, str | None]:
    ...
    for row in rows:
        ...
        if nq == code or nq == name:
            ...
            scored.append((2, row))
    scored.sort(key=lambda row: (row[0], row[1]['price_rub'], row[1]['name']))
    if not scored:
...
def show_admin_pack_picker(target, page: int = 0) -> None:
    ...
    for row in chunk:
        ...
        lines.append(f"• {row['name']} ({row['code']}) · {row['price_rub']} ₽ · {state}")
...
def callbacks(call) -> None:
    ...
    if data == 'admin:packs' and uid == OWNER_ID:
        ...
        kb = InlineKeyboardMarkup(row_width=1)
        for row in rows:
            lines.append(f"• {row['code']} · {row['name']} · {row['price_rub']}₽ · {'вкл.' if row['enabled'] else 'выкл.'}")
