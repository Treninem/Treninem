...
# Ещё 50 рецептов продвинутого эндгейма.
while rid <= 370:
    mode = rid % 5
    if mode == 0:
        out = equipment[(rid * 7) % len(equipment)]
        ing = {
            materials[(rid * 2) % len(materials)]: 3,
            materials[(rid * 5) % len(materials)]: 3,
            elixirs[(rid * 3) % len(elixirs)]: 1,
            scrolls[(rid * 2) % len(scrolls)]: 1,
        }
        station = 'forge'
        title = f"Апекс-кузня #{rid}"
    elif mode == 1:
        out = elixirs[(rid * 5) % len(elixirs)]
        ing = {
            foods[(rid * 2) % len(foods)]: 2,
            materials[(rid * 3) % len(materials)]: 2,
            recipes[(rid * 4) % len(recipes)]: 1,
        }
        station = 'alchemy'
        title = f"Высшая алхимия #{rid}"
    elif mode == 2:
        out = scrolls[(rid * 5) % len(scrolls)]
        ing = {
            recipes[(rid * 2) % len(recipes)]: 1,
            materials[(rid * 4) % len(materials)]: 2,
            elixirs[(rid * 3) % len(elixirs)]: 1,
        }
        station = 'scribe'
        title = f"Рунописец апекса #{rid}"
    elif mode == 3:
        out = foods[(rid * 5) % len(foods)]
        ing = {
            foods[(rid * 2) % len(foods)]: 2,
            materials[(rid * 3) % len(materials)]: 2,
            elixirs[(rid * 2) % len(elixirs)]: 1,
        }
        station = 'kitchen'
        title = f"Полевая кухня мастера #{rid}"
    else:
        out = materials[(rid * 7) % len(materials)]
        ing = {
            materials[(rid * 2) % len(materials)]: 4,
            materials[(rid * 9) % len(materials)]: 2,
            scrolls[(rid * 3) % len(scrolls)]: 1,
        }
        station = 'craft'
        title = f"Сборка трофеев #{rid}"
    add_recipe(rid, title, out, 1 + (1 if rid % 9 == 0 else 0), ing, station=station, required_level=8 + rid // 12)
    rid += 1
