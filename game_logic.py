...
def item_effect_text(item_id: int) -> str:
    ...
    if item["slot"]:
        parts.append(f"Слот: {item['slot']}")
    if item["stats"]:
        parts.append("Бонусы: " + ", ".join(f"{k}+{v}" for k, v in item["stats"].items() if v))
    if item["hp_restore"] or item["energy_restore"]:
        parts.append(f"Восстановление: HP +{item['hp_restore']}, энергия +{item['energy_restore']}")
    buffs = item.get("buffs", {})
    clean_buffs = {k: v for k, v in buffs.items() if v and k != "duration_min"}
    if clean_buffs:
        parts.append("Эффекты: " + ", ".join(f"{k} {v}" for k, v in clean_buffs.items()))
...
def expedition_success_chance(level: int, difficulty: str, stats: dict[str, Any]) -> float:
    base = {"easy": 0.60, "normal": 0.42, "hard": 0.28, "nightmare": 0.16}[difficulty]
    per_level = {"easy": 0.016, "normal": 0.011, "hard": 0.009, "nightmare": 0.008}[difficulty]
    stat_bonus = min(0.18, (stats["attack"] + stats["speed"] + stats["luck"]) / 5000)
    talent_bonus = stats.get("expedition_pct", 0) / 100
    chance = base + max(0, level - 1) * per_level + stat_bonus + talent_bonus
    cap = {"easy": 0.985, "normal": 0.93, "hard": 0.88, "nightmare": 0.84}[difficulty]
    return max(0.1, min(cap, chance))
...
def dungeon_success_chance(level: int, difficulty: str, floor: int, stats: dict[str, Any], modifier: dict[str, Any]) -> float:
    base = {"easy": 0.70, "medium": 0.48, "hard": 0.28}[difficulty]
    lvl_factor = max(0, level - floor) * {"easy": 0.012, "medium": 0.010, "hard": 0.008}[difficulty]
    combat_score = (stats["attack"] * 1.2 + stats["defense"] + stats["speed"] * 0.7 + stats["luck"] * 0.4) / (210 + floor * 12)
    mod = modifier.get("monster_def_pct", 0) / 100
    chance = base + lvl_factor + combat_score * 0.04 - mod * 0.2 + stats.get("dungeon_pct", 0) / 100
    cap = {"easy": 0.985, "medium": 0.91, "hard": 0.82}[difficulty]
    return max(0.08, min(cap, chance))
