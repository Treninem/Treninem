...
PET_SPECIES = {
    "spark": {
        ...
        "role": "талисман удачи",
        "bonus": {"luck_pct": 4},
        "ability_name": "Искра везения",
        "ability_desc": "Повышает шанс на приятные находки, редкие награды и удачные исходы в приключениях.",
    },
    "mole": {
        "title": "Крот-рудокоп",
        "emoji": "🦦",
        "role": "добытчик ресурсов",
        "bonus": {"material_drop_pct": 8},
        "ability_name": "Глубокий ход",
        "ability_desc": "Лучше чувствует руду и материалы, поэтому чаще приносит ресурсы из экспедиций и подземелий.",
    },
    "owl": {
        "title": "Лесная сова",
        "emoji": "🦉",
        "role": "учёный разведчик",
        "bonus": {"xp_pct": 6},
        "ability_name": "Мудрый круг",
        "ability_desc": "Помогает быстрее учиться на боях и вылазках, увеличивая получаемый опыт.",
    },
    "beetle": {
        "title": "Бронежук",
        "emoji": "🪲",
        "role": "защитник",
        "bonus": {"defense_pct": 6},
        "ability_name": "Панцирный заслон",
        "ability_desc": "Укрепляет хозяина и делает его устойчивее в затяжных боях и тяжёлых походах.",
    },
    "ferret": {
        "title": "Шустрый хорёк",
        "emoji": "🦫",
        "role": "ловкач",
        "bonus": {"speed_pct": 6},
        "ability_name": "Рывок тени",
        "ability_desc": "Добавляет скорости, чтобы раньше ходить в бою и увереннее проходить рискованные вылазки.",
    },
    "frog": {
        "title": "Ядовитая квакша",
        "emoji": "🐸",
        "role": "мастер состояний",
        "bonus": {"status_pct": 6},
        "ability_name": "Едкая слизь",
        "ability_desc": "Усиливает эффекты состояний, ядов и ослаблений на врагах.",
    },
}
...
# Ещё по 100 предметов в каждую основную категорию.
EXTRA_FOOD_STYLE = ["Сытный", "Пикантный", "Копчёный", "Янтарный", "Травяной", "Пряный", "Лунный", "Грозовой", "Тёплый", "Охотничий"]
...
for i in range(100):
    ...
    buffs = {
        ...
        "speed_pct": rarity * 2 if i % 6 == 2 else 0,
        "luck_pct": rarity * 2 if i % 6 == 3 else 0,
        "xp_pct": rarity * 2 if i % 6 == 4 else 0,
        "gold_gain_pct": rarity * 2 if i % 6 == 5 else 0,
        "duration_min": 18 + rarity * 8,
    }
    add_item(
        "food", rarity, 261 + i, name,
        emoji="🍲" if i % 2 == 0 else "🥪",
        price=16 + rarity * 6 + i,
        weight=1 + rarity // 2,
        hp_restore=18 + rarity * 7 + i % 5,
        energy_restore=16 + rarity * 6 + (i * 2) % 7,
        buffs=buffs,
        description="Сытная еда с кратковременными усилениями для походов и боёв.",
        tags=["еда", "баф", "полевая кухня"],
    )

EXTRA_MATERIAL_STYLE = ["Полярный", "Дикий", "Сумеречный", "Солнечный", "Буревой", "Шахтёрский", "Тотемный", "Глубинный", "Ритуальный", "Кристальный"]
...
for i in range(100):
    ...
    add_item(
        ...
        price=12 + rarity * 7 + i,
        weight=2 + rarity,
        description="Продвинутый материал для ремесла, улучшений и редких обменов.",
        tags=["материал", "редкий ресурс", "крафт"],
    )

EXTRA_EQUIP_STYLE = ["Лунный", "Яростный", "Багряный", "Штормовой", "Тотемный", "Стражевой", "Туманный", "Имперский", "Следопытский", "Клыкастый"]
EXTRA_EQUIP_KIND = ["шлем", "доспех", "наручи", "поножи", "амулет", "маска", "плащ", "пояс", "перчатки", "талисман"]
extra_slots = ["head", "body", "paws", "legs", "accessory"]
for i in range(100):
    rarity = 1 + min(5, i // 17)
    ...
    stats = {
        ...
        "attack": rarity * (2 if slot in {"paws", "accessory"} else 1) + (1 if i % 5 == 0 else 0),
        "defense": rarity * (2 if slot in {"head", "body"} else 1) + (1 if i % 4 == 0 else 0),
        "speed": max(0, rarity - 1 + (1 if slot in {"legs", "accessory"} else 0)),
        "luck": max(0, rarity - 2 + (1 if slot == "accessory" else 0)),
    }
    buffs = {
        ...
        "speed_pct": rarity * 2 if i % 5 == 2 else 0,
        "luck_pct": rarity * 2 if i % 5 == 3 else 0,
        "status_pct": rarity * 2 if i % 5 == 4 else 0,
    }
    add_item(
        "equipment", rarity, 261 + i, name,
        emoji="🛡️" if slot in {"head", "body"} else "⚔️",
        price=80 + rarity * 24 + i * 2,
        weight=3 + rarity,
        slot=slot,
        stats=stats,
        buffs=buffs,
        max_durability=110 + rarity * 28,
        description=f"Продвинутый предмет экипировки для слота: {SLOT_TITLES[slot]}.",
        tags=["экипировка", slot, "снаряжение"],
    )

EXTRA_ELIXIR_STYLE = ["Вихревой", "Крепкий", "Точный", "Тёплый", "Ледяной", "Лунный", "Острый", "Царский", "Сумрачный", "Рассветный"]
...
for i in range(100):
    ...
    buffs = {
        ...
        "speed_pct": rarity * 3 if i % 6 == 2 else 0,
        "luck_pct": rarity * 3 if i % 6 == 3 else 0,
        "xp_pct": rarity * 3 if i % 6 == 4 else 0,
        "gold_gain_pct": rarity * 3 if i % 6 == 5 else 0,
        "duration_min": 24 + rarity * 10,
    }
    add_item(
        "elixir", rarity, 261 + i, name,
        emoji="🧪" if i % 2 == 0 else "⚗️",
        price=40 + rarity * 18 + i * 2,
        weight=1,
        hp_restore=10 + rarity * 6 if i % 2 == 0 else 0,
        energy_restore=10 + rarity * 6 if i % 2 == 1 else 0,
        buffs=buffs,
        description="Продвинутый эликсир с сильными временными эффектами.",
        tags=["эликсир", "алхимия", "баф"],
    )

EXTRA_SCROLL_STYLE = ["Печать", "Свиток", "Руна", "Грамота", "Кодекс", "Знамение", "Хартия", "Память", "Печать стаи", "Складка"]
EXTRA_SCROLL_KIND = ["натиска", "стойкости", "удачи", "мастерства", "обмена", "следа", "охоты", "теней", "дороги", "перерождения"]
extra_scroll_effects = [
    {"slots_plus": 10}, {"weight_plus": 300}, {"attack_pct": 10}, {"defense_pct": 10}, {"speed_pct": 10},
    {"luck_pct": 10}, {"steal": 1}, {"spy": 1}, {"gift_bonus": 1}, {"reroll": 1},
    {"expedition_pct": 8}, {"dungeon_pct": 8}, {"boss_damage_pct": 8}, {"inventory_slots": 12}, {"max_weight": 600},
]
for i in range(100):
    rarity = 1 + min(5, i // 17)
    name = f"{EXTRA_SCROLL_STYLE[i % len(EXTRA_SCROLL_STYLE)]} {EXTRA_SCROLL_KIND[(i * 2) % len(EXTRA_SCROLL_KIND)]} №{i + 1}"
    buffs = {**extra_scroll_effects[i % len(extra_scroll_effects)], "duration_min": 18 + rarity * 12}
    add_item(
        "scroll", rarity, 261 + i, name,
        emoji="📜" if i % 3 else "🗞️",
        price=48 + rarity * 20 + i * 2,
        weight=1,
        buffs=buffs,
        description="Свиток с прикладным или боевым эффектом, который действует ограниченное время.",
        tags=["свиток", "эффект", "одноразовый"],
    )

EXTRA_RECIPE_STYLE = ["Полевой", "Туманный", "Старинный", "Кузнечный", "Алхимический", "Договорной", "Императорский", "Архивный", "Запретный", "Звёздный"]
EXTRA_RECIPE_KIND = ["чертёж", "манускрипт", "кодекс", "лист", "план", "свод", "рецепт", "свиток"]
for i in range(100):
    rarity = 1 + min(5, i // 17)
    name = f"{EXTRA_RECIPE_STYLE[i % len(EXTRA_RECIPE_STYLE)]} {EXTRA_RECIPE_KIND[(i * 4) % len(EXTRA_RECIPE_KIND)]} №{i + 1}"
    add_item(
        "recipe", rarity, 261 + i, name,
        emoji="📘" if i % 2 == 0 else "🗂️",
        price=30 + rarity * 14 + i,
        weight=1,
        description="Расширенный рецепт для продвинутых ремесленных цепочек и коллекций.",
        tags=["рецепт", "ремесло", "схема"],
    )
